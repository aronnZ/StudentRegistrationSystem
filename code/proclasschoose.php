	...
	<script>
		{
			...
			var $JudgeFlage; //判断是否关闭注册
			xhr.open('post','JudgeShutDown.php');

			xhr.onload = function()
			{
				//console.log(xhr.responseText);
				JudgeFlage = xhr.responseText;
			}
					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="row">
									<div class="col-xs-12">
										<h3 class="header smaller lighter blue">课程列表</h3>
										...
										<div class="table-responsive">
											<table id="sample-table-2" class="table table-striped table-bordered table-hover">
												...
												if(!$con)
												{
													echo
													"
													<Script>alert('数据库连接失败！')</Script>
													";
												}

												else
												{
													$sql="select dept_name from professor where prof_id='$user'";
													$result = mysqli_query($con,$sql);
													$row = mysqli_fetch_assoc($result);
													$dept = $row["dept_name"];

													$sql="select year,semester from administrator";
													$result = mysqli_query($con,$sql);
													$row = mysqli_fetch_assoc($result);
													$year = $row["year"];	
													$semester = $row["semester"];	

													$sql="select sec_id,title,dept_name,building,day,start_hr,start_min,end_hr,end_min,semester  from course natural join section natural join time_slot where dept_name='$dept' and year='$year' and semester='$semester' and sec_id not in ( SELECT sec_id from teaches )";
													$result = mysqli_query($con,$sql);
													$rows = mysqli_fetch_all($result);
														...
														<tr>
																											...
																											<td>
																												<div class='visible-md visible-lg hidden-sm hidden-xs action-buttons'>
																													<a class='blue' href='#'>
																														<i class='icon-plus bigger-130' id = '$num' onclick ='add()'></i>
																													</a>
																												</div>
...
<![endif]-->
		...
		<script>
			function add()
			{
				if(JudgeFlage!=1) //未关闭注册
				{
					var name = "row"+event.target.id+'0';
				
				var id = document.querySelector("span[name="+name+"]").innerHTML;

				var r = window.confirm("您确定要教授这门课程吗");
				
				if(r==true)
				{
					var xhr = new XMLHttpRequest();

					xhr.open('post','proaddclass.php');
					...
					xhr.onload = function()
					{
						...
						{
							alert("添加成功！");
							location.href("proclasschoose.php");
						}
						else
						{
							alert(result);

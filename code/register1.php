...
$dep =array("考古学院","美术学院","计算机科学与技术学院","机械学院","生物学院","体育学院","外国语学院","物理学院");
if($getID==""){
    //新建人员
    $ID= 1;
    if (!$result) {
    ...
    if($M=='S'){
        ...
        $sql="INSERT INTO student  VALUES ('".$ID."',NULL,'".$name."',";
        ...
        $sql="INSERT INTO professor  VALUES ('".$ID."',NULL,'".$name."',";
        if($birthday) $sql=$sql."'".$birthday."','".$SSN."',";
        else $sql=$sql."NULL,'".$SSN."',";
        if($status and ($status!="undefined")) $sql=$sql."'".$status."',";
        else $sql=$sql."NULL,";
    ...
    if($result) {
        ...
        if ($M == 'P') {
            ...
            if($extrainfo =="Mechanics") $output.="机械学院";
            if($extrainfo =="Biology") $output.="生物学院";
            if($extrainfo =="P.E.") $output.="体育学院";
    ...
    if($del!=""){
        ...
        $ID=$getID;
        if($M=='S'){
            $sql="DELETE FROM student where stu_id = '".$ID."' and SSN = '".$SSN."';";
        }else if($M='P'){
            $sql="DELETE FROM professor where prof_id = '".$ID."' and SSN = '".$SSN."';";
        }
        //echo $sql;
